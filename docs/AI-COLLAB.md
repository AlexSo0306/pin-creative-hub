...
容器右上角飘着一个 **18×17 的黑色圆块**。套件没报，因为它当时没有断言盯着特效配色。
→ 又一次印证：**「断言通过」不等于「看过了」**。

**根因（一条被忽略的契约）**：mood-mates 的引擎**不带样式表**。
特效层 `core/fx.js` 建图元时**只挂类名、不给 `fill`**：
...
当初按「只搬引擎、跳过 `site/`」的原则引入，就漏掉了这段 —— 于是图元落到
SVG 默认值 `fill:#000`。
→ **教训：「引入一个引擎」不等于「拷贝它的 core/」。类名是宿主必须兑现的样式契约，
   它可能被写在仓库的另一个目录里。** 已把整条契约写进
   `public/js/vendor/mood-mates/README.md`（含 6 个类名的逐个说明）。

**改了什么**：`public/css/mascot.css` 新增「引擎样式契约」段落 ——
补 `mm-bubble` / `mm-sheen` / `mm-speck` 三个类 + 线稿模式 4 条；
颜色按锅宝的蓝重配（上游 `#C6D6F5` 是配紫白系角色的），并按明暗主题分两档。
另把 `.guobao-slot` 加 `overflow: hidden`：与 `.guobao-stage svg { overflow: visible }`
**成对**使用 —— svg 那层放开让泡泡不被 193px 舞台硬切，slot 这层收住让泡泡不飘到导航文字上。
（不会裁掉状态气泡：气泡是 `position: fixed`，祖先 overflow 管不到它。）

**⭐ 最有价值的一段：新断言第一次跑就暴露了自己是空转的。**
新加的断言是「特效粒子有配色，没落到黑色兜底」。按纪律做注入对照
（`INJECT_FX_BLACK=1` 把变量强制成黑）复现现场，结果：

```
✓ 特效粒子有配色，没落到黑色兜底 — 黑色 0 / 0 个     ← 绿着，但一个都没查到
✗ 特效粒子确实出现（否则下一条会空转） — 命中 0 个
```

**粒子是瞬态的**：cloudpuff 的泡泡走完 `travel + hang + pop` 就被移除。
第一版写的是「`setEmotion('33')` → `sleep(1500)` → 点采一次」，
注入那次正好撞在「已经放完了」的空档上 → 集合为空 → 「没有黑色」判绿，**实际什么都没测**。
→ 改成 `celebrate()` 触发后 **26 次 × 120ms 轮询取峰值**，
并把 `峰值 > 0` 也写进第二条的条件里，**集合为空时绝不允许判绿**。
→ **通用规则：瞬态动画的断言必须轮询采样；而「计数类」断言必须配一条「集合非空」的兄弟断言，
   否则它会在空集上安静地变绿。**

**实测证据**（无头 Chrome + CDP，真实应用）：
- `verify-guobao.mjs` **27/27 通过**（新增 2 条），
  粒子峰值 **24 个**，`mm-bubble=rgb(214,232,255)` / `mm-sheen=rgba(255,255,255,0.52)`
- **注入对照**：`INJECT_FX_BLACK=1` → **26/1**，第 2 条如期变红（黑色 24/峰值 24）
  —— 这条守卫**被证明会失败**，不是「没失败过」
- 浅色主题 `mm-bubble=rgb(187,216,245)`（分档生效）
- `emotion-sheet.html` 32/32 格渲染、零报错
- 残留 Chrome 进程 0

**新增工具**：`tools/mascot-verify/capture-eyeball.mjs` —— 人工目检取图
（32 表情接触表 + 侧栏容器 scale 4 特写 + 浅色主题特写）。**不参与断言，只给人眼看。**
一次性诊断探针用完即删，没有入库。

**遗留（待 Alex 定）**：
1. 旧自研引擎 `public/js/components/mascot/`（6 模块 108KB）+ `mascot.js` 已不在页面上，
   但 `demo.html` / `sheet.html` / `verify.mjs` / `verify-plugin.mjs` 仍指向它
   —— **`verify-plugin.mjs` 现在必然失败**。已在工具链 README 顶部加「两代引擎并存」对照表。
2. 渐变：品牌规范规则 1「不使用渐变」针对的是 **UI 表面**（背景/卡片）；
   角色插画保留了引擎的体积渐变 + 釉面高光。若要一律拉平，说一声。

> ⚠️ **一旦这个工作台变成对外产品/服务，就必须先取得上游商业授权，或把引擎换成自研实现。**
> 依据 `public/js/vendor/mood-mates/legal/LICENSE` 第 3 / 6 条，不要凭记忆判断。
